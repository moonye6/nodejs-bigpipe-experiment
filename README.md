...
>这里原文是使用的node+jade的方式实现bigpipe，考虑到ejs更常用，故做了一个调整，[原文](https://github.com/undoZen/nodejs-bigpipe-experiment)链接
...
我会用 express 来演示，简单起见，我们选用 ejs 作为模版引擎，并且我们不使用引擎的子模版（partial）特性，而是以子模版渲染完成以后的 HTML 作为父模版的数据。
...
运行 npm install 安装这三个库，这里使用ejs作为模板
...
```
之前我们说“以子模版渲染完成以后的 HTML 作为父模版的数据”，指的就是这样，`temp.s1` 和 `temp.s2` 两个方法会生成 s1.ejs和 s2.ejs两个文件的 HTML 代码，然后把这两段代码作为 layout.ejs里面 s1、s2 两个变量的值。
...
在接下来的优化之前，我们加入 jquery 库并把 css 样式放到外部文件，在包含 app.js 的目录下运行：
```
mkdir static
cd static
```
并且把 layout.ejs中的 style 标签里的代码拿出来放到 static/style.css 里，然后把 head 标签改为：
...
如果我们一收到 HTTP 请求就把 head 部分返回，然后两个 section 等到异步操作结束后再返回，这是利用了 HTTP 的[分块传输编码](https://zh.wikipedia.org/wiki/%E5%88%86%E5%9D%97%E4%BC%A0%E8%BE%93%E7%BC%96%E7%A0%81)机制。在 node.js 里面只要使用 res.write() 方法就会自动加上 `Transfer-Encoding: chunked` 这个 header 了。这样就能在浏览器加载静态文件的同时，node 服务器这边等待异步调用的结果了，我们先删除 layout.ejs中的这 section 这两行：
```
<section id="s1"><%-s1%></section>
<section id="s2"><%-s2%></section>
```
因此我们在 res.render() 里也不用给 { s1: …, s2: … } 这个对象，并且因为 res.render() 默认会调用 res.end()，我们需要手动设置 render 完成后的回调函数，在里面用 res.write() 方法。layout.ejs 的内容也不必在 writeResult() 这个回调函数里面，我们可以在收到这个请求时就返回，注意我们手动添加了 content-type 这个 header：
...
需要注意的是，要写入的网页片段有 script 标签的情况，如将 s1.ejs改为
...
```
还要在 layout.ejs 把两个 section 添加回来：
...
本文的代码在 https://github.com/moonye6/nodejs-bigpipe-experiment ，我把每一步做成一个 commit 了，希望你 clone 到本地实际运行并 hack 一下看看。因为后面几步涉及到加载顺序了，确实要自己打开浏览器才能体验到而无法从截图上看到（其实应该可以用 gif 动画实现，但是我懒得做了）。
